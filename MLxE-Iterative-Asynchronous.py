...
the MLxE - Iterative-Asynchronous Architecture
    ...
    - Each Executor geretates examples till all Executors gererate at least one example in the iteration
...
import tensorflow_addons as tfa # for 
...
args = {# SYSTEM Settings
        ...
        # GYM Settings
        "max_episodes": 3000, # How many Episodes do you want to run?
...
def LMxE(worker,
    ...
    if worker >= 0:
        ...
        if worker == 0: #            
            ...
            print("Saved Model", worker, len(exemplar_model))
            lock.release()

            memory_buffer = np.full(state_n+4,0.0)
            counter_learninig = 0
            
        episod_acomplished_indicator = 0 # to report if executor finalized at least one episod in given iteration

        while episod_counter.value < max_episodes:
            
            episod_acomplished_indicator = 0 # to report if executor finalized at least one episod in given iteration
            while executor_counter.value < executors_n:
                ...
                while done == False and internal_step_counter <= internal_step_counter_limit and executor_counter.value < executors_n:
                    ...
                    R_pos = 1*(1-np.abs(next_state[0])/2.4) # 2.4 max value ### !!! in documentation says 4.8 but failes beyound 2.4
                    R_ang = 1*(1-np.abs(next_state[2])/0.20943951023931953) ### !!! in documentation says 0.418 max value
                    ...
                    #if internal_step_counter % experience_batch_size == 0 or done == True:
                    if done == True or internal_step_counter == internal_step_counter_limit:    
                        
                        observations = observations[-np.minimum(observations.shape[0], 1024):]
                        ...
                        # Test modofication for implementation -- to remove unfinalized episodes. 
                        if episod_acomplished_indicator == 1 and executor_counter.value == executors_n:
                            pass
                        else:
                            ...
                            print("Ending Episod on Executor:", worker, "Episod", episod_counter.value, "Initial State", pos_start, "Steps:", internal_step_counter,executor_counter.value)
                        
                        observations = np.empty(state_n+3)
                        
                if episod_acomplished_indicator == 0:
                    lock.acquire()    
                    executor_counter.value += 1
                    lock.release()
                    
                    episod_acomplished_indicator = 1
                ...
                if internal_step_counter_best.value >= 50000:
                    print("\nREACHED GOAL of 50K Steps in, 'counter_learninig' variabel not available", "Learning Iterations in",time.time()-time_start, "seconds \n")
            ...
            # Assign MEMORIZER to worker 0
                ...
                while collect_obs.qsize() > 0:
                    ...
                    #Test Modification for iterative asynchronious implementation
...
if __name__ == "__main__":
    ...
    collect_obs = manager.Queue() # Collector of Episods from EXEMPLARS
