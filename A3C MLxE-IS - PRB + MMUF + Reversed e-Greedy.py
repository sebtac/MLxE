...
args = {# SYSTEM Settings
        ...
        # GYM Settings
        "max_episodes": 500, # How many Episodes do you want to run?
...
def LMxE(worker,
    ...
    epsilon_decay_policy = args["epsilon_decay_policy"]
    ...
    if worker >= 0:
        ...
        while episod_counter.value < max_episodes:
            ...
            while done == False and internal_step_counter <= internal_step_counter_limit:
                ...
                if epsilon_decay_policy == 1:
                    #epsilon *= (2-epsilon_decay)
                    epsilon *= epsilon_decay
                    if epsilon >= np.random.rand(): # Random-Informed
            ...
            if worker == 0:
                ...
                memory_buffer[:,-1] = np.average(memory_buffer[:,[-(pmb_cols-2),-(pmb_cols-3)]], axis = 1 ) # Best! 280-310
                #memory_buffer[:,-1] = memory_buffer[:,-(pmb_cols)] # DR - Not Good, training collapsed
                #memory_buffer[:,-1] = memory_buffer[:,-(pmb_cols-2)] # Age - Good, 310 Games
                #memory_buffer[:,-1] = memory_buffer[:,-(pmb_cols-3)] # TD-Error ???
